...
class StateActionProvider:
    ...
    async def on_trigger(self, data: dict) -> None:
        ...
        changes = {
            **self.data.get("data", {}),
            **{key: data.get(ref) for key, ref in self.data.get("var-data", {}).items()}
        }


        LOGGER.debug("State action triggered %s %s", changes, target.identifier)
...
class Module:
    ...
    async def start(self, event: str) -> None:
        ...
        await self.core.event_engine.gather("gather_automation_providers",
                                            engine=self,
